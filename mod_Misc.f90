...
FUNCTION IsABoson(Type)! only counts photon,Z,W,Higgs as bosons, and not the gluon
implicit none
logical IsABoson
integer :: Type

   if( abs(Type).eq.11 .or. abs(Type).eq.12 .or. abs(Type).eq.13 .or. abs(Type).eq.26 ) then
      IsABoson = .true.
   else
      IsABoson = .false.
   endif
...
SUBROUTINE SwitchCouplings(couplin,couplout)
  implicit none
  real(8), intent(in)  :: couplin
  real(8), intent(out) :: couplout
  
  couplout=couplin

end SUBROUTINE SwitchCouplings
...
!-----------``scalar product'' for the two spinors
      ...
      function vbqV(sp,e1,coupl_left,coupl_right,HDcoupl_left,HDcoupl_right,q,tag)
      implicit none
      complex(8), intent(in) :: e1(:)
      complex(8), intent(in) :: sp(:)
      ...
      complex(8) :: vbqV(size(sp)),ubarqslZsl(size(sp)),ubarZslqsl(size(sp)),ubarsig(size(sp))
      integer :: Ds,ColorlessParticle
!  complex(8) :: check_new(size(sp)),check_old(size(sp))

      if (.not. present(tag)) then
         ColorlessParticle=1    ! default is SM vector+axial vector (i.e. for SM Z)
      else
         ColorlessParticle=tag
      endif
      

      if (ColorlessParticle .eq. 2) then
         if (.not. present(HDcoupl_left) .or. .not. present(HDcoupl_right)) then
            call Error("Coupling to q-dependent currents not specified in vbqV")
         endif
         if (.not. present(q)) then
            call Error("q argument not passed in vbqV")
         endif
      endif
         
      Ds = size(sp)
 
      if (ColorlessParticle .eq. 1) then               ! this is as for Z (or Z prime??)
      ...
      elseif (ColorlessParticle .eq. 2) then              ! see RR notes
         ubarZslqsl=spb2_(spb2_(sp,e1),q)                 ! ubar Zslash qslash  = ubar Z_mu gamma^mu q_nu gamma^nu
         ubarqslZsl=spb2_(spb2_(sp,q),e1)                 ! ubar qslash Zslash  = ubar q_mu gamma^mu Z_nu gamma^nu
...
! print *, "new vbqV",vbqV
      ...
      function vVq(e1,sp,coupl_left,coupl_right,HDcoupl_left,HDcoupl_right,q,tag)
      implicit none
      complex(8), intent(in) :: e1(:)
      complex(8), intent(in) :: sp(:)
      complex(8), intent(in) :: coupl_left,coupl_right
      ...
      complex(8), intent(in), optional :: q(:)
      integer, intent(in), optional :: tag
      complex(8) :: vVq(size(sp)),qslZslv(size(sp)),Zslqslv(size(sp)),sigv(size(sp))
      integer :: Ds,ColorlessParticle
      ...
      if (ColorlessParticle .eq. 2) then
         if (.not. present(HDcoupl_left) .or. .not. present(HDcoupl_right)) then
            call Error("Coupling to q-dependent currents not specified in vVq")
         endif
         if (.not. present(q)) then
            call Error("q argument not passed in vVq")
         endif
      endif
...
!             vVq = -(0d0,1d0)*( coupl_left*Chir_check(.true., spi2_(e1,sp)) + coupl_right*Chir_check(.false., spi2_(e1,sp)) )
      ...
      elseif (ColorlessParticle .eq. 2) then
         Zslqslv=spi2_(e1,spi2_(q,sp))
         qslZslv=spi2_(q,spi2_(e1,sp))
...
!           phase = -pol*(cphi + (0d0,1d0)*sphi)
          ...
          f(4)=dcmplx(-st/dsqrt(2d0),0d0)                      !*phase
            ...
            if( abs(pol).ne.1) call Error("wrong helicity setting in pol_mless",pol)
